// ==========================================================================
// Blog Writer Agent — pure handler module (called by api/agents/[name].js)
// ==========================================================================
...
export async function handle(body) {
    const { brand = 'marcos', topic, transcript, category = 'ai-strategy' } = body || {};

    if (!VALID_BRANDS.includes(brand)) throwHttp(400, `brand must be one of: ${VALID_BRANDS.join(', ')}`);
    if (!topic && !transcript) throwHttp(400, 'Either topic or transcript is required');
    if (!VALID_CATEGORIES.includes(category)) throwHttp(400, `category must be one of: ${VALID_CATEGORIES.join(', ')}`);
